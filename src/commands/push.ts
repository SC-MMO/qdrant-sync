...
export async function runPush(file: string | null = null): Promise<void> {
	...
	const existingCollectionNames = new Set(response.collections.map((collection) => collection.name));

	const oldData: CanonicalCollections = await client.readCollectionConfigurations();

	for (const [collectionName, newCollection] of Object.entries(data)) {
		if (existingCollectionNames.has(collectionName)) {
			await client.updateCollectionConfiguration(collectionName, newCollection, oldData[collectionName]);
		} else {
