...
import { DEFAULT_CONFIG, DEFAULT_ENV_EXAMPLE, DEFAULT_SCHEMA } from '../helpers/defaultExamples';
...
export function runInit(): void {
	...
	const existingFiles = [configPath, schemaPath, envExamplePath].filter(existsSync);

	if (existingFiles.length > 0) {
		console.error('Error: Initialization aborted because the following file(s) already exist:');
