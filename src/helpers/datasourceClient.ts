...
function getPayloadSchema(collection: CanonicalCollection | undefined): PayloadSchema {
	return collection?.payload_schema ?? {};
}

function isSameFieldSchema(a: PayloadSchema[string] | undefined, b: PayloadSchema[string] | undefined): boolean {
...
function toCreateCollectionParams(collection: CanonicalCollection): CreateCollectionParams {
...
export class QdrantSyncClient extends QdrantClient {
	...
	public async readCollectionConfigurations(): Promise<Record<string, CanonicalCollection>> {
		const collections: CollectionInfo[] = (await this.getCollections()).collections.filter((collection) => {
		...
		const entries: [string, CanonicalCollection][] = enrichedCollections.map((collection) => {
